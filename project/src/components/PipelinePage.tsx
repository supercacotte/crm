...
const PipelinePage: React.FC<PipelinePageProps> = ({
  ...
  const prospectsByStatus = {
    ...
    closed_won: filteredProspects.filter(p => p.status === 'closed_won'),
    closed_lost: filteredProspects.filter(p => p.status === 'closed_lost')
  };

  // Stats
  const totalProspects = prospects.length;
  const activeProspects = prospects.filter(p => !['closed_won', 'closed_lost'].includes(p.status)).length;
  ...
  const getKpiData = () => {
    ...
    const activeCount = scopedProspects.filter(p => !['closed_won', 'closed_lost'].includes(p.status)).length;
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              <option value="closed_won">Clients Signés</option>
          ...
          <div className="p-6">
            <div className="flex gap-6 overflow-x-auto pb-4">
              ...
              <KanbanColumn
                ...
                prospects={prospectsByStatus.closed_won}
                color="bg-green-100"
              />
            </div>
          </div>
